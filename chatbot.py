...
def get_recipe_name(user_input):
    ...
    recipe_name = name_response.candidates[0].content.parts[0].text
    return recipe_name

def get_recipe_steps(recipe_name):
    recipe_steps_response = get_gemini_response(f"Can you generate numbered recipe steps for the recipe {recipe_name}?")
    recipe_steps = recipe_steps_response.candidates[0].content.parts[0].text
    return recipe_steps

def output_recipe(recipe_name, recipe_steps):
...
st.write(" We can generate recipes across 100+ cuisines for you based on the ingredients you mention below.")
...
if submit and user_input:
    ...
    output_recipe(recipe_name, recipe_steps)
